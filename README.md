...
[**Blocks**](https://github.com/fastygo/blocks) and **applications** need **copyable, stable widgets** with clear contracts — not page-specific markup scattered across products.
...
| **Elements** (this repo) | **Copy-first widgets** — behavior + composition; **JS** extends `ui8kit` with **APG-aligned** patterns where the kit stops. |
| **Blocks** | Page organisms built directly on UI8Kit. |
...
- **Copy-first dependencies:** Elements may depend on **UI8Kit** only — not on Blocks, other Elements, apps, or workspace-local packages. See [`.cursor/rules/elements-layer.mdc`](.cursor/rules/elements-layer.mdc).
- **Go / templ:** use only explicit Tailwind utility classes that pass `.ui8px` policy.
